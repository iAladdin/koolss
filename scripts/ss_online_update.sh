...
add_ss_servers(){
	...
	echo_date 添加SS节点：$remarks $server $server_port $encrypt_method $password $group $1 >> $LOG_FILE
	dbus set ssconf_basic_name_$ssindex=$remarks
	[ -n "$1" ] && dbus set ssconf_basic_group_$ssindex=$group
	...
	dbus set ssconf_basic_max_node=$ssindex
}
...
remove_ss_node_gap(){
	...
	#echo_date 现有节点顺序：$SEQ >> $LOG_FILE
	echo_date 最大SS节点序号：$MAX >> $LOG_FILE
	echo_date SS节点数量：$NODE_NU >> $LOG_FILE
	...
	dbus set ssconf_basic_node_max=$NODE_NU
	dbus set ssconf_basic_max_node=$NODE_NU
}
...
get_oneline_rule_now(){
	...
	if [ "$?" == "0" ];then
		...
		if [ -n "$NODE_FORMAT1" ];then
			...
			urllinks=$(cat /tmp/ssr_subscribe_file_temp1.txt | sed 's/ssr:\/\///g')
			...
			do
				if [ -n "`echo -n "$link" | grep "#"`" ]; then
					...
					remarks=`echo -n "$link" | awk -F'#' '{print $2}'`
				else
					new_sslink=`echo -n "$ssrlink" | sed 's/ss:\/\///g'`
					remarks='AddByLink'
				fi
				decode_ss_url_link $new_sslink 1
				echo_date hasCleanThisGroup $hasCleanThisGroup >> $LOG_FILE
				if [ $hasCleanThisGroup == "-1" ];then
					echo_date 首次清理本地订阅服务器... >> $LOG_FILE
					LOCAL_GROUPS_INDEX=`dbus list ssconf_basic_group|grep "$group"|cut -d "_" -f 4|cut -d "=" -f 1|sort -n`
					if [ -n "$LOCAL_GROUPS_INDEX" ];then
							echo_date 清理$group 订阅服务器... >> $LOG_FILE
							for GROUP_INDEX in $LOCAL_GROUPS_INDEX
							do
								dbus remove ssconf_basic_name_$GROUP_INDEX
								dbus remove ssconf_basic_group_$GROUP_INDEX
								dbus remove ssconf_basic_mode_$GROUP_INDEX
								dbus remove ssconf_basic_server_$GROUP_INDEX
								dbus remove ssconf_basic_port_$GROUP_INDEX
								dbus remove ssconf_basic_method_$GROUP_INDEX
								dbus remove ssconf_basic_password_$GROUP_INDEX
								dbus remove ssconf_basic_password_$GROUP_INDEX
							done
							remove_ss_node_gap
					fi
